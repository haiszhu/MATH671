...
% function mainfmg()
%
% full multigrid for -u_xx-u_yy = f on [0,1]x[0,1], problem setup and
% initialization. The coarse grid finite difference 5-point stencil is
% generated directly.



N = 256;                % grid size (power of 2)
...
u = fmg(A, f, u, h, alpha, beta, smoothnum, matrix );
rnorm  = norm( residual(A,f,u,h) )*h
enorm  = norm( u-uexact )*h


end

function u = fmg(A, f, u, h, alpha, beta, smoothnum, matrix)


l = 1;
[ny(l),nx(l)] = size(f);    % number of grid
hval(l) = h;                % mesh space
eval(['f_' num2str(l) ' = f;']);
eval(['u_' num2str(l) ' = u;']);
eval( ['A_' num2str(l) ' = A;' ]);
...
while (mod(nx(l)-1,2)==0 && mod(ny(l)-1,2)==0 && nx(l)-1>2 && ny(l)-1>2 )
   ...
   % grid size at level l down from the finest grid 
   l = l+1;
   nx(l) = (nx(l-1)-1)/2 + 1;
   ny(l) = (ny(l-1)-1)/2 + 1;
   hval(l) = 2*hval(l-1);
   
   % form mesh on level l
   dxl = alpha*hval(l); dyl = beta*hval(l);
   xl = 0:dxl:1; yl = 0:dyl:1;
   [Xl,Yl] = meshgrid(xl,yl);
   
   % form f, u, and A at all level
   eval(['f_' num2str(l) ' = ffun(Xl,Yl);']);
   %ftemp = eval(['f_',num2str(l-1)]);
   %eval(['f_' num2str(l) ' = f2c(ftemp);']);
   eval(['u_' num2str(l) ' = zeros(ny(l),nx(l));']);
   eval( ['A_' num2str(l) ' = fd5point(ny(l),nx(l),alpha,beta);']);
end
nl = l;

% form u on coarsest level
uc = eval(['u_' num2str(nl)]);
fc = eval(['f_' num2str(nl)]);
Ac = eval(['A_' num2str(nl)]);
uc = mgv(Ac, fc, uc, hval(nl), alpha, beta, smoothnum, matrix);
eval(['u_' num2str(nl) ' = uc;']);

% go upwards towards finest level 
for l=nl-1:-1:1
   uc = eval(['u_' num2str(l+1)]);
   ul = eval(['u_' num2str(l)]);
   ul = c2f( uc );
   fl = eval(['f_' num2str(l)]);
   Al = eval(['A_' num2str(l)]);
   
   ul = mgv(Al, fl, ul, hval(l), alpha, beta, smoothnum, matrix);
